...
async def install(event):
    ...
    _o = o.split("\n")
    o = "\n".join(_o)
    OUTPUT = f"List of Plugins in kaalBOT :- \n\n{o}\n\n<><><><><><><><><><><><><><><><><><><><><><><><>\nHELP:- If you want to know the commands for a plugin, do :- \n.plinfo <plugin name> without the < > brackets. \nJoin https://t.me/kaalsupport01 for help."
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "cmd_list.text"
            kaal_file = await bot.send_file(
                event.chat_id,
                out_file,
                force_document=True,
                allow_cache=False,
            ...
            await edit_or_reply(kaal_file, f"Output Too Large. This is the file for the list of plugins in kaalBOT.\n\n**BY :-** {DEFAULTUSER}")
            await event.delete()
