...
async def _(event):
    ...
    kaalevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_skaal(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    kaaluser = await event.client.get_me()
    if kaaluser.username:
        curruser = kaaluser.username
    else:
        curruser = "kaalBOT"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    kaalevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on kaalBOT server and shows details'
).add_command(
