...
async def echo(kaal):
    ...
    if kaal.reply_to_msg_id is not None:
        ...
        try:
            ...
            aura = Get(aura)
            await kaal.client(aura)
        except BaseException:
            pass
        if is_echo(user_id, chat_id):
            await edit_or_reply(kaal, "The user is already enabled with echo ")
            return
        addecho(user_id, chat_id)
        await edit_or_reply(kaal, "Hii....😄🤓")
    else:
...
async def echo(kaal):
    ...
    if kaal.reply_to_msg_id is not None:
        ...
        if is_echo(user_id, chat_id):
            ...
            await edit_or_reply(kaal, "Echo has been stopped for the user")
        else:
...
async def echo(kaal):
    ...
    if len(output_str) > Config.MAX_MESSAGE_SIZE_LIMIT:
        ...
        await edit_or_reply(kaal, reply_text)
    else:
        await edit_or_reply(kaal, output_str)


@bot.on(events.NewMessage(incoming=True))
async def samereply(kaal):
    if kaal.chat_id in Config.UB_BLACK_LIST_CHAT:
        return
    if is_echo(kaal.sender_id, kaal.chat_id):
        ...
        try:
            ...
            pass
        if kaal.message.text or kaal.message.sticker:
            await kaal.reply(kaal.message)
