...
# porting to hellbot by @Murat_30_kaal...
...
async def get_users(event):
    ...
    if not sender.id == me.id:
        kaal = await edit_or_reply(event, "`processing...`")
    else:
        kaal = await edit_or_reply(event, "`processing...`")
    aura = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await kaal.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await kaal.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(aura.full_chat.id):
        try:
            if error.startswith("Too"):
                return await kaal.edit(
