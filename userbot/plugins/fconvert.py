...
from kaalBOT.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(kaal):
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await kaal.client.download_media(
...
async def _(event):
    if event.fwd_from:
        return
    kaalreply = await event.get_reply_message()
    if not kaalreply or not kaalreply.media or not kaalreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if kaalreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    kaalevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, kaalreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await kaalevent.edit("`This file is not supported`")
            kaalresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            kaalfile = Path(await event.client.download_media(kaalresponse, "./temp/"))
            kaalgif = Path(await unzip(kaalfile))
            ...
            for files in (kaalgif, kaalfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await kaalevent.edit("Unblock @tgstogifbot")
            return
